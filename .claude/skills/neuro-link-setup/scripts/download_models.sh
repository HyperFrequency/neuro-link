...
# NLR_MODELS_DIR matches pkg/docker/compose.yaml's bind-mount source for
# the llama-embed service. Defaulting to $REPO_ROOT/models keeps a
# monorepo install self-contained; setting NLR_MODELS_DIR=/some/abs/path
# (e.g. for an external SSD) puts the artifacts there AND tells compose
# to mount the same path.
MODELS_DIR="${NLR_MODELS_DIR:-$REPO_ROOT/models}"
...
# --- Persist resolved Octen settings as a manifest ---
# pkg/docker/compose.yaml env_file's this manifest into the llama-embed
# service so the variant the user downloaded with is the SAME variant
# compose serves. Without this, drift is possible (different shells, env
# unset on one side, etc.). The manifest is a normal env_file: KEY=VALUE
# per line, no quoting, no comments after values.
MANIFEST="$MODELS_DIR/.octen-manifest.env"
{
  echo "# Generated by download_models.sh on $(date -u +%Y-%m-%dT%H:%M:%SZ)"
  echo "# Single source of truth for Octen artifact selection — read by"
  echo "# pkg/docker/compose.yaml's llama-embed service via env_file."
  echo "OCTEN_VARIANT=$OCTEN_VARIANT"
  echo "NLR_MODELS_DIR=$MODELS_DIR"
} > "$MANIFEST"
echo
